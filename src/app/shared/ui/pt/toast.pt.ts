...
export const FloraToastPT = {
  ...
  message: ({ instance }: { instance?: { message?: { severity?: string | null } } } = {}) => ({
    class: [
      ...
      {
        'bg-white dark:bg-neutral-800 border-success-500 text-neutral-800 dark:text-neutral-100': instance?.message?.severity === 'success',
        'bg-white dark:bg-neutral-800 border-danger-500 text-neutral-800 dark:text-neutral-100':  instance?.message?.severity === 'error',
        'bg-white dark:bg-neutral-800 border-warning-500 text-neutral-800 dark:text-neutral-100': instance?.message?.severity === 'warn',
        'bg-white dark:bg-neutral-800 border-neutral-300 dark:border-neutral-600 text-neutral-800 dark:text-neutral-100': !instance?.message?.severity || instance.message.severity === 'info',
      },
    ],
  }),
