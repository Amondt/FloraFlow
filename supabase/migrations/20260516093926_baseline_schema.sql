...
CREATE POLICY "Gardeners manage their own profile" ON public.profiles FOR ALL USING (auth.uid () = id)
WITH
  CHECK (auth.uid () = id);
...
CREATE POLICY "Gardeners can manage their own zones completely" ON public.zones FOR ALL USING (auth.uid () = user_id)
WITH
...
CREATE POLICY "Gardeners can manage their own specific plants" ON public.plants FOR ALL USING (auth.uid () = user_id)
WITH
  CHECK (auth.uid () = user_id);
...
CREATE POLICY "Anyone authenticated can view cached botanical species profiles" ON public.cached_botanical_records FOR
SELECT
  USING (auth.role () = 'authenticated');

-- Writes are locked to internal edge operations only (service role bypasses RLS)
CREATE POLICY "Only internal edge operations can populate botanical indices" ON public.cached_botanical_records FOR ALL USING (FALSE)
WITH
  CHECK (FALSE);
...
BEFORE UPDATE ON public.zones FOR EACH ROW
EXECUTE FUNCTION public.update_updated_at ();

CREATE TRIGGER trg_plants_updated_at
BEFORE UPDATE ON public.plants FOR EACH ROW
EXECUTE FUNCTION public.update_updated_at ();
...
CREATE OR REPLACE FUNCTION public.snooze_plant_check (p_plant_id UUID, p_days INT) RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
