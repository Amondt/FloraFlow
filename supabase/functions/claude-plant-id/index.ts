...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: cachedRows } = await supabase
      ...
      .select('scientific_name, inat_taxon_id')
      .in('scientific_name', allScientificNames);

    const cacheMap = new Map<string, { inat_taxon_id: number | null }>(
      (cachedRows ?? []).map((r) => [r.scientific_name, { inat_taxon_id: r.inat_taxon_id }]),
    );
    ...
    // 8. Respond — inat_taxon_id is null when enrichment is still pending for the primary match
