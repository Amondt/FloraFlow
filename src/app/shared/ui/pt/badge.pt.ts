...
export const FloraTagPT = {
  root: ({ instance }: { instance?: { severity?: string | null } } = {}) => ({
    class: [
      ...
      {
        'bg-primary-50 text-primary-700 dark:bg-primary-900/30 dark:text-primary-300': !instance?.severity || instance.severity === 'info',
        'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300':         instance?.severity === 'success',
        'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300':     instance?.severity === 'warn',
        'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300':                 instance?.severity === 'danger',
        'bg-neutral-100 text-neutral-600 dark:bg-neutral-700 dark:text-neutral-300':    instance?.severity === 'secondary',
      },
    ],
  }),
  icon:  { class: 'text-xs' },
